        ...
        <div className='flex flex-col'>
          <div className="border-2 border-indigo-500 rounded-t-md bg-primary p-4 w-128 overflow-y-scroll flex flex-col h-128 max-w-7xl min-w-7xl">
            {messages.map(({ id, text, uid }) => (
              <div>
                <div key={id} className={`flex p-5 space-x-1 items-center my-2 rounded-full ${uid === auth.currentUser.uid ? 'rounded-br-3xl bg-[#37257f] flex-row-reverse float-right text-white' : 'rounded-bl-3xl bg-[#fafafa] float-left'}`}>
